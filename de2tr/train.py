...
from de2tr.config import BaseOptions
from de2tr.start_end_dataset import \
    StartEndDataset, SyntheticDataset, start_end_collate, prepare_batch_inputs
from de2tr.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from de2tr.inference import eval_epoch, start_inference, setup_model
from utils.basic_utils import AverageMeter, dict_to_markdown
from utils.model_utils import count_parameters

from de2tr.loss_fun.VTCLoss import VTCLoss
from de2tr.loss_fun.CTCLoss import CTC_Loss

# Single GPU. For multiple GPUs, e.g. os.environ['CUDA_VISIBLE_DEVICES'] = '0,2,3'
...
def start_training():
    ...
    print('##################')
    if opt.a_feat_dir is None:
        print("No use audio_feats")
    else:
        print(f"Use audio_feats: {opt.a_feat_dir}")
    print('##################')
    if opt.a_feat_dir is None:
        dataset_config = dict(
        ...
        if opt.use_synthetic_data:
            # BAS: the original samples plus the same samples with the background
            # replaced by clips from an unrelated video (see SyntheticDataset).
            train_dataset_1 = StartEndDataset(**dataset_config)
            train_dataset_2 = SyntheticDataset(**dataset_config, synth_prob=opt.synth_prob)
            train_dataset = ConcatDataset([train_dataset_1, train_dataset_2])
            print(f"Created a combined training dataset with {len(train_dataset)} samples.")
            print(f"({len(train_dataset_1)} original + {len(train_dataset_2)} augmented)")
        else:
