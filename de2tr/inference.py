...
from de2tr.config import TestOptions
from de2tr.model import build_model
from de2tr.span_utils import span_cxw_to_xx
from de2tr.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from de2tr.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from de2tr.postprocessing_de2tr import PostProcessorDETR
...
def _is_optional_saliency_key(key):
    """Parameters that differ between the released checkpoints but never affect the
    forward pass.

    The older checkpoint predates the saliency-prior modules, so it has neither
    `decoder.saliency_proj1/2` (aliases of the top-level projections, which it stores
    under `saliency_proj1/2`) nor the `decoder.layers.*.saliency_cls_film` FiLM, which is
    zero-initialised and only applied when the saliency prior is enabled. A checkpoint's
    layout is recorded by `sal_priori_film` in its opt.json; either layout loads into
    either build, and the mismatch is reported as a warning.
    """
    if "saliency_cls_film" in key:
        return True
    return key.startswith(("transformer.decoder.saliency_proj1.",
                           "transformer.decoder.saliency_proj2."))
...
def setup_model(opt):
    ...
    if opt.resume is not None:
        ...
        fatal_unexpected = [k for k in unexpected if not _is_optional_saliency_key(k)]
        if fatal_unexpected:
            raise RuntimeError(
                f"Checkpoint has keys the model does not define: {sorted(fatal_unexpected)}")
        fatal_missing = [k for k in missing if not _is_optional_saliency_key(k)]
        if fatal_missing:
            raise RuntimeError(
                f"Model parameters absent from checkpoint: {sorted(fatal_missing)}")
        if missing or unexpected:
            logger.warning(
                f"Loaded with {len(missing)} missing and {len(unexpected)} unused key(s) "
                f"that are inert saliency-prior parameters: "
